 ...
 * filesystem in the RP23XX flash memory chip.  It uses the space not
 ...
 * binary is built (using tools/rp23xx/make_flash_fs.c), but any changes
 ...
 *       rapid; because of how the RP23XX's flash access routines work with
...
#define FLASH_START_OFFSET (rp23xx_smart_flash_start - (uint8_t *)XIP_BASE)
#define FLASH_END_OFFSET   (rp23xx_smart_flash_end   - (uint8_t *)XIP_BASE)
#define FLASH_START_READ   (rp23xx_smart_flash_start + 0x03000000)
...
#define FLASH_BLOCK_COUNT  (CONFIG_RP23XX_FLASH_LENGTH - FLASH_START_OFFSET)\
                           / FLASH_BLOCK_SIZE

/* Sectors are the smallest unit that can be written */

#define FLASH_SECTOR_SIZE  256
#define FLASH_SECTOR_COUNT (CONFIG_RP23XX_FLASH_LENGTH - FLASH_START_OFFSET)\
                           / FLASH_SECTOR_SIZE
...
static int     rp23xx_flash_erase      (struct mtd_dev_s *dev,
                                        off_t             startblock,
                                        size_t            nblocks);

static ssize_t rp23xx_flash_block_read (struct mtd_dev_s *dev,
                                        off_t             startblock,
                                        size_t            nblocks,
                                        uint8_t          *buffer);

static ssize_t rp23xx_flash_block_write(struct mtd_dev_s *dev,
                                        off_t             startblock,
                                        size_t            nblocks,
                                        const uint8_t    *buffer);

static ssize_t rp23xx_flash_byte_read  (struct mtd_dev_s *dev,
                                        off_t             offset,
                                        size_t            nbytes,
                                        uint8_t          *buffer);

static int     rp23xx_flash_ioctl      (struct mtd_dev_s *dev,
                                        int              cmd,
                                        unsigned long    arg);
...
extern const uint8_t rp23xx_smart_flash_start[256];
extern const uint8_t rp23xx_smart_flash_end[0];

/****************************************************************************
 * Private Data
 ****************************************************************************/

static struct rp23xx_flash_dev_s my_dev =
{
  .mtd_dev =
  {
    rp23xx_flash_erase,
    rp23xx_flash_block_read,
    rp23xx_flash_block_write,
    rp23xx_flash_byte_read,
...
static int     rp23xx_flash_erase(struct mtd_dev_s *dev,
                                  ...
                                  size_t            nblocks)
{
  rp23xx_flash_dev_t *rp_dev = (rp23xx_flash_dev_t *)dev;
  irqstate_t          flags;
  int                 ret    = OK;
...
/****************************************************************************
 * Name: rp23xx_flash_initialize
 *
 * Description: Bind a block mode driver that uses the built-in rp23xx
...
struct mtd_dev_s *rp23xx_flash_mtd_initialize(void)
{
  ...
  if (memcmp(rp23xx_smart_flash_start, "2040", 4) == 0)
    {
      ...
      do_erase(FLASH_END_OFFSET,
                CONFIG_RP23XX_FLASH_LENGTH - FLASH_END_OFFSET);

      /* Erase the "magic" flag, setting the first two bytes to zero. */

      memcpy(buffer, rp23xx_smart_flash_start, FLASH_SECTOR_SIZE);
