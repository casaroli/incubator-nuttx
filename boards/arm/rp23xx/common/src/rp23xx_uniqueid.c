...
void rp23xx_flash_cmd(const uint8_t *txbuf, uint8_t *rxbuf, size_t count);
...
noinline_function locate_code(".ram_code.rp23xx_flash_cmd")
void rp23xx_flash_cmd(const uint8_t *txbuf, uint8_t *rxbuf, size_t count)
{
...
{
  uint64_t x;
  uint8_t  txbuf[RP23XX_FLASH_ID_BUFFER_SIZE];
  uint8_t  rxbuf[RP23XX_FLASH_ID_BUFFER_SIZE];

  memset(g_uniqueid, 0xac, CONFIG_BOARDCTL_UNIQUEID_SIZE);
  memset(txbuf, 0, RP23XX_FLASH_ID_BUFFER_SIZE);
  memset(rxbuf, 0, RP23XX_FLASH_ID_BUFFER_SIZE);
  txbuf[0] = RP23XX_FLASH_RUID_CMD;

  rp23xx_flash_cmd(txbuf, rxbuf, RP23XX_FLASH_ID_BUFFER_SIZE);

  /* xorshift PRNG: */

  x = *(uint64_t *)(rxbuf + RP23XX_FLASH_ID_BUFFER_OFFSET);
